...
const langData = {
  vi_VN: {
    ...
    "tosscoin.invalidChoice":
      "Lựa chọn không hợp lê, các lựa chọn có sẵn:\n{validChoices}",
  ...
  en_US: {
    ...
    "tosscoin.invalidChoice":
      "Invalid choice, available choices:\n{validChoices}",
  ...
  ar_SY: {
    ...
    "tosscoin.invalidChoice":
      "خيار غير صالح ، الخيارات المتاحة:\n{validChoices}",
...
async function onCall({ message, args, extra, getLang }) {
  ...
  if (!choice || !validChoices.includes(choice)) {
    ...
    return message.reply(
      getLang("tosscoin.invalidChoice", { validChoices: validStr }),
    );
  }
  ...
  try {
    ...
    if (bet < BigInt(extra.minbet)) {
      return message.reply(
        getLang("tosscoin.minmoney", { minBet: extra.minbet }),
      );
    }
